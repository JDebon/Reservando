...
Listado.prototype.buscarRestaurante = function(id) {
    let resultado = this.restaurantes.find((element) => element.id === id);
...
function eliminarRepeticion(array) {
    let arrayFiltrado = array.filter(function(elem, index, self) {
...
Listado.prototype.obtUbicacion = function() {
    //Array donde se van a ir agregando las ciudades (van a estar repetidas)
    var arrayUbicacion = this.restaurantes.map((element) => element.ubicacion);
    let arrayFiltrado = eliminarRepeticion(arrayUbicacion);

    return arrayFiltrado.sort();
};

//Obtiene todos los rubros de los restaurantes sin repetidos. Su funcionamiento es similar a obtC()
Listado.prototype.obtRubro = function() {
    var arrayRubro = this.restaurantes.map((element) => element.rubro);
    let arrayFiltrado = eliminarRepeticion(arrayRubro);
...
Listado.prototype.obtenerRestaurantes = function(filtroRubro, filtroCiudad, filtroHorario) {
    ...
    if (filtroRubro !== null) {
        restaurantesFiltrados = restaurantesFiltrados.filter(
            (restaurant) => restaurant.rubro == filtroRubro
        );
    }

    if (filtroCiudad !== null) {
        restaurantesFiltrados = restaurantesFiltrados.filter(
            (restaurant) => restaurant.ubicacion == filtroCiudad
        );
    }
